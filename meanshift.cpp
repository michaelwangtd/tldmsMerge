...
//Mat MeanShift::Calpdfmodel(Mat frame, Rect rec)
{
	...
	for (int i = 0; i < rows; i++)
	{
		...
		for (int j = 0; j < cols; j++)
		{
			...
			//tempb = pb[col_index] / 15;
			//tempg = pg[col_index] / 15;
			//tempr = pr[col_index] / 15;

			pmixImg[j] = tempb * 256 + tempg * 16 + tempr;		//使用公式blue*256+green*16+red，但是需要先把各种颜色划分成16个区间
			//pmixImg[j] = tempb * 256 + tempg * 15 + tempr;
...
Mat candidate_model = Calpdfmodel(next_frame, _target_region);//根据当前帧和上一帧的目标区域计算直方图模型
...
bool MeanShift::mtrack(Mat next_frame)
{
	//Rect next_rec;
	cout << "MeanShift start track:mtrack" << endl;

	for (int iter = 0; iter < cfg._Maxiter; iter++)
	{

		printf("iterater num:%d\n", iter );
		printf("_target_region:%d %d %d %d\n", _target_region.x, _target_region.y, _target_region.width, _target_region.height);

		_next_rec = _target_region;
		float delta_x = 0.0;
		float delta_y = 0.0;
		float normlized_x;
		float normlized_y;
		...
		cout << "calpdfmodel func calculate start" << endl;

		Mat candidate_model = Calpdfmodel(next_frame, _target_region);//根据当前帧和上一帧的目标区域计算直方图模型

		cout << "calpdfmodel func calculate finish" << endl;
		cout << "_flag_overflow" << _flag_overflow << endl;

		if (_flag_overflow)
			//break;
			cout << "!_flag_overflow == true!" << endl;
			return false;

		cout << "Calweight func start calculate" << endl;
		Mat weight = Calweight(_target_region, _target_model, candidate_model);
		cout << "Calweight func end calculate" << endl;
		...
		_next_rec.x += static_cast<float>(delta_x / sum_weight*center_col);	//之前因为进行了归一化，注意这里的实际偏移量要乘回center_row
		_next_rec.y += static_cast<float>(delta_y / sum_weight*center_row);

		if (abs(_next_rec.x - _target_region.x) < 1 && abs(_next_rec.y - _target_region.y) < 1)
			//break;
			return false;
		else
		{
			_target_region.x = _next_rec.x;
			_target_region.y = _next_rec.y;
		}
	}
	
	cout << "MeanShift end track:mtrack" << endl;
