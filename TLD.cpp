...
//void TLD::processFrame(const cv::Mat& img1, const cv::Mat& img2, const cv::Mat& img3,
	//BoundingBox& bbnext, bool& lastboxfound, bool tl, FILE* bb_file){

void TLD::processFrame(const cv::Mat& img1, const cv::Mat& img2, const cv::Mat& img3,
	BoundingBox& bbnext, bool& lastboxfound, bool tl, FILE* bb_file,MeanShift & ms){

	/*这个函数有两个目的：1）当前帧中确定出当前目标框bbnext
	2）完成当前帧的跟踪后，将目标框赋值给lastbox变量，为下一帧的跟踪做准备
	*/
...
void TLD::track(const Mat& img3,MeanShift & ms){

	Mat img4 = img3;

	if (!tracked){
		//重新初始化_target_model模板
		cout << "ms restart init _target_model" << endl;

		ms.Ini_target_frame(img4, lastbox);
	}

	//确定上一帧的目标框
	ms._target_region = lastbox;

	tracked = ms.mtrack(img4);
	...
	if (tracked){
		...
		if (tbb.x>img4.cols || tbb.y>img4.rows || tbb.br().x < 1 || tbb.br().y <1){
			tvalid = false; //too unstable prediction or bounding box out of image
			tracked = false;
			//printf("Too unstable predictions FB error=%f\n", tracker.getFB());
			return;
		...
		bb.x = max(tbb.x, 0);
		bb.y = max(tbb.y, 0);
		bb.width = min(min(img4.cols - tbb.x, tbb.width), min(tbb.width, tbb.br().x));
		bb.height = min(min(img4.rows - tbb.y, tbb.height), min(tbb.height, tbb.br().y));
		getPattern(img4(bb), pattern, mean, stdev);
		vector<int> isin;
		float dummy;
		classifier.NNConf(pattern, isin, dummy, tconf); //Conservative Similarity
		tvalid = lastvalid;
